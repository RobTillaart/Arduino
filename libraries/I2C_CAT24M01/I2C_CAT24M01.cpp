...
I2C_CAT24M01::I2C_CAT24M01(uint8_t deviceAddress, TwoWire * wire)
{
  ...
  _deviceSize = I2C_DEVICESIZE_CAT24M01;
  _pageSize   = I2C_PAGESIZE_CAT24M01;
  _wire = wire;
}


bool I2C_CAT24M01::begin(int8_t writeProtectPin)
{
  _lastWrite = 0;
...
uint8_t I2C_CAT24M01::getAddress()
{
...
int I2C_CAT24M01::writeByte(const uint32_t memoryAddress, const uint8_t data)
{
...
//  returns I2C status, 0 = OK
int I2C_CAT24M01::setBlock(const uint32_t memoryAddress, const uint8_t data, const uint32_t length)
{
...
//  returns I2C status, 0 = OK
int I2C_CAT24M01::writeBlock(const uint32_t memoryAddress, const uint8_t * buffer, const uint32_t length)
{
...
uint8_t I2C_CAT24M01::readByte(const uint32_t memoryAddress)
{
  uint8_t rdata;
...
//  returns bytes read.
uint32_t I2C_CAT24M01::readBlock(const uint32_t memoryAddress, uint8_t * buffer, const uint32_t length)
{
...
//  returns 0 == OK
int I2C_CAT24M01::updateByte(const uint32_t memoryAddress, const uint8_t data)
{
...
uint32_t I2C_CAT24M01::updateBlock(const uint32_t memoryAddress, const uint8_t * buffer, const uint32_t length)
{
  uint32_t addr = memoryAddress;
  uint32_t len = length;
  uint32_t rv = 0;
...
bool I2C_CAT24M01::writeByteVerify(const uint32_t memoryAddress, const uint8_t value)
{
...
//  return false if write or verify failed.
bool I2C_CAT24M01::writeBlockVerify(const uint32_t memoryAddress, const uint8_t * buffer, const uint32_t length)
{
...
//  return false if write or verify failed.
bool I2C_CAT24M01::setBlockVerify(const uint32_t memoryAddress, const uint8_t value, const uint32_t length)
{
...
//  return false if write or verify failed.
bool I2C_CAT24M01::updateByteVerify(const uint32_t memoryAddress, const uint8_t value)
{
...
//  return false if write or verify failed.
bool I2C_CAT24M01::updateBlockVerify(const uint32_t memoryAddress, const uint8_t * buffer, const uint32_t length)
{
...
//  METADATA SECTION
//
uint32_t I2C_CAT24M01::getDeviceSize()
{
  return _deviceSize;
}


uint16_t I2C_CAT24M01::getPageSize()
{
  return _pageSize;
}


uint32_t I2C_CAT24M01::getLastWrite()
{
  return _lastWrite;
}


void I2C_CAT24M01::setExtraWriteCycleTime(uint8_t ms)
{
  _extraTWR = ms;
}


uint8_t I2C_CAT24M01::getExtraWriteCycleTime()
{
  return _extraTWR;
}
...
//  WRITEPROTECT
//
bool I2C_CAT24M01::hasWriteProtectPin()
{
  return (_writeProtectPin >= 0);
}


void I2C_CAT24M01::allowWrite()
{
...
void I2C_CAT24M01::preventWrite()
{
  if (hasWriteProtectPin())
  {
...
void I2C_CAT24M01::setAutoWriteProtect(bool b)
{
  if (hasWriteProtectPin())
  {
    _autoWriteProtect = b;
...
bool I2C_CAT24M01::getAutoWriteProtect()
{
...
int I2C_CAT24M01::_pageBlock(uint32_t memoryAddress, const uint8_t * buffer, const uint16_t length, const bool incrBuffer)
{
  uint32_t addr = memoryAddress;
  uint16_t len = length;
  while (len > 0)
  {
    uint16_t bytesUntilPageBoundary = this->_pageSize - addr % this->_pageSize;
...
//  returns 0 = OK otherwise error
int I2C_CAT24M01::_WriteBlock(uint32_t memoryAddress, const uint8_t * buffer, const uint8_t length)
{
  _waitEEReady();
...
//  returns bytes read
uint8_t I2C_CAT24M01::_ReadBlock(uint32_t memoryAddress, uint8_t * buffer, const uint8_t length)
{
  _waitEEReady();
...
void I2C_CAT24M01::_waitEEReady()
{
  //  Wait until EEPROM gives ACK again.
  //  this is a bit faster than the hardcoded 5 milliSeconds
  //  TWR = WriteCycleTime
