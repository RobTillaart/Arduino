...
Used to configure the PCA9685 general behaviour. Check datasheet for details.
...
- **uint8_t setPWM(uint8_t channel, uint16_t onTime, uint16_t offTime)** The chip has 16 channels to do PWM.
...
- **uint8_t setFrequency(uint16_t freq, int offset = 0)** set the update speed of the channels. 
This value is set the same for all channels at once.
- **uint16_t getFrequency(bool cache = true)** get the current update frequency of the channels. 
...
- **int lastError()** returns **PCA9685_OK = 0** if all is OK, and other error codes otherwise.
...
- Method 1 is a tested method which is specific to the PCA9685.
