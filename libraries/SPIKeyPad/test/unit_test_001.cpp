...
//  AUTHOR: Rob Tillaart
//    DATE: 2026-04-09
// PURPOSE: unit tests for the SPIKeyPad library
...
unittest_setup()
{
  fprintf(stderr, "SPI_KEYPAD_LIB_VERSION: %s\n", (char *) SPI_KEYPAD_LIB_VERSION);
}
...
{
  assertEqual(16, SPI_KEYPAD_NOKEY);
  assertEqual(17, SPI_KEYPAD_FAIL);
  assertEqual(255, SPI_KEYPAD_THRESHOLD);

  assertEqual(44, SPI_KEYPAD_4x4);
  assertEqual(53, SPI_KEYPAD_5x3);
  assertEqual(62, SPI_KEYPAD_6x2);
  assertEqual(81, SPI_KEYPAD_8x1);
}
...
unittest(test_constructor)
{
  ...
  SPIKeyPad keyPad(KEYPAD_ADDRESS);

  assertEqual(SPI_KEYPAD_NOKEY, keyPad.getLastKey());
...
// {
  ...
  // // assertEqual(SPI_KEYPAD_NOKEY, keyPad.getKey());
