//
//    FILE: moduloMap_wire_cutter.ino
//  AUTHOR: Rob Tillaart
// PURPOSE: demo math for wire cutter application
...
//  assume we want to cut off 40 centimetre every time.
float wireLength = 40;
...
void setup()
{
  ...
  float rotations = wireCutter.rotations(wireLength);

  //  how many steps are this? (note we round up)
  uint16_t steps = ceil(rotations * stepsPerRotation);
