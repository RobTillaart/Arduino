...
As a PCF8575 has 16 lines up to 5 rotary encoders with a switch can be read over I2C.
...
Typical rotary encoders generate 1, 2 or 4 steps per click (see issue #3).
...
Since 0.1.2 the library provides **getClicks()** and a **setStepsPerClick()**
...
In #3 this problem is discussed (a bit) and in essence it can be interpreted 
...
- **bool begin(uint8_t deviceCount = 5)** UNO ea. initializes the class. 
...
rotary encoder pins.
The parameter pin is not verified if it is within range (0..7).
...
TODO get numbers right (for now based upon related libraries)
...
The example **rotaryDecoderSwitch5_demo_interrupt.ino** shows a possible solution 
