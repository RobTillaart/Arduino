...
bool rotaryDecoderSwitch5::begin(uint8_t deviceCount)
{
...
bool rotaryDecoderSwitch5::reset(uint8_t re)
{
...
//  CLICKS API 0.4.2 #14
//
int32_t rotaryDecoderSwitch5::getClicks(uint8_t re)
{
  if (re >= ROTDEC_MAX_COUNT) return 0;
  //  test if 1?
...
bool rotaryDecoderSwitch5::setClicks(uint8_t re, int32_t clicks)
{
  return setValue(re, clicks * _stepsPerClick[re]);
}

//  configure per channel.
bool rotaryDecoderSwitch5::setStepsPerClick(uint8_t re, uint8_t spc)
{
  if (re >= ROTDEC_MAX_COUNT) return false;
...
uint8_t rotaryDecoderSwitch5::getStepsPerClick(uint8_t re)
{
  if (re >= ROTDEC_MAX_COUNT) return 0;
